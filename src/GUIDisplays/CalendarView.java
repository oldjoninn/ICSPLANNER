...
public class CalendarView {
   ...
   public Scene createCalendarScene() {
      ...
      // 2. DYNAMIC SIZING: Get Screen Dimensions
      Rectangle2D screenBounds = Screen.getPrimary().getVisualBounds();
      double screenWidth = screenBounds.getWidth();
      double screenHeight = screenBounds.getHeight();
    
      // 3. Calendar Content Layer
      ...
      // --- Build Calendar UI ---
      HBox topBar = createTopBar(screenWidth);
      calendarContent.setTop(topBar);
    
      GridPane calendarGrid = createCalendarGrid(screenWidth);
      ...
      dashboardPane.prefHeightProperty().bind(mainRoot.heightProperty());
      dashboardPane.setTranslateX(-SIDEBAR_WIDTH); // Hide initially
    
      // 5. Add all layers to main root
      mainRoot.getChildren().addAll(calendarContent, overlay, dashboardPane);
    
      // 6. ANIMATION: Fade in on load
   ...
   private Map<String, CourseTimeSlot> organizeSchedule() {
      ...
      for (Course course : student.getCoursesTaken()) {
          ...
          if (days == null || time == null || days.equals("TBA") || time.equals("TBA")) continue;
   ...
   private VBox createMergedCell(CourseTimeSlot slot, double width) {
      ...
      Label secLabel = new Label(course.getSection());
