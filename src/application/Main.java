...
public class Main extends Application {
   ...
   public void start(Stage primaryStage) {
       ...
       // Just use StackPane directly as root - it automatically centers content
       Pane root = new Pane();
       ...
       fadeIn.setNode(welcomeText); // The node to animate
       fadeIn.setFromValue(0.0);   // Start invisible (0% opacity)
       fadeIn.setToValue(1.0);     // End visible (100% opacity)
       fadeIn.play();              // Start the animation
     
       FadeTransition fadeIn2 = new FadeTransition();
       fadeIn2.setDuration(Duration.millis(4500)); //
       fadeIn2.setNode(abbrievText); // The node to animate
       fadeIn2.setFromValue(0.0);   // Start invisible (0% opacity)
       fadeIn2.setToValue(1.0);     // End visible (100% opacity)
       fadeIn2.play();              // Start the animation
       ...
       // Add both to StackPane (background first, then positioned content)
       ...
       //root.getChildren().addAll(backgroundImage, positionedContainer);
   ...
   private static void loadDegreePrograms() {
       ...
       DegreeProgram bscs = new DegreeProgram("BSCS", "applications/ics_cmsc_courses.csv");
       DegreeProgram mscs = new DegreeProgram("MSCS", "applications/ics_mscs_courses.csv");
       DegreeProgram mit = new DegreeProgram("MIT", "applications/ics_mit_courses.csv");
       DegreeProgram phd = new DegreeProgram("PHD", "applications/ics_phd_courses.csv");
       bscs.loadCourses();
       mscs.loadCourses();
       mit.loadCourses();
       phd.loadCourses();
   ...
   private static void loadCourseOfferings() {
       CSVReader.readCourseOfferings("applications/course_offerings.csv", courseOfferings);
   }
